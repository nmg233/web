...
面向“大中小贯通科创育人”项目的 PBL（项目式学习）本地数字化管理平台。平台聚焦线下课堂后的数字化巩固与成果转化：执行导师编排课程和学习内容、导入选课并完成评审，学生依次完成课堂回顾、知识卡片与练习、学习报告与反思，教师以只读观察者身份查看明确分配学生的成长结果。

> 支持本地开发与服务器部署，部署步骤见文末“服务器部署”章节。

灵境小智的快速设置与本地数据共享说明见 [AI 配置指南](AI配置指南.md)。
...
| `AI_CONFIG_SECRET` | 启用 AI 前必需 | 无 | 64 位十六进制独立密钥，用于加密数据库中的 DeepSeek API Key |
| `AI_ALLOWED_BASE_URLS` | 否 | `https://api.deepseek.com` | 管理员可选的 HTTPS API 地址白名单，逗号分隔 |
...
| 执行导师 `academic_mentor` | 管理自己创建或受邀授课的课程、课时、知识卡片、小练习、任务和选课；评审课程范围内的作品与学习报告；管理相关学生的成长档案 |
| 教师 `teacher` | 纯观察者：只读查看由管理员明确分配（`users.teacher_id`）学生的学习进展、已通过成果、报告、反思和成长轨迹；不进入课程执行或评审链路 |
| 学生 `student` | 学习已报名且已发布的课程，完成课堂回顾、知识卡片与练习、学习报告与反思，查看个人成长档案并使用实验工具 |
| 新媒体 `media` | 查看基础工作台和已发布课程列表，使用帮助与反馈、通知中心；不进入课程执行、学生管理和评审链路 |

### 用户权限矩阵

下表描述当前代码实际生效的权限。`管理`表示允许读写，但仍受对应数据范围限制；`只读`表示不能通过该模块新增、修改、删除或评审；`本人`表示仅能操作当前账号自己的数据；`—`表示前端无业务入口，后端也不授予对应写权限。

| 功能模块 | 管理员 `admin` | 执行导师 `academic_mentor` | 教师 `teacher` | 学生 `student` | 新媒体 `media` |
| --- | --- | --- | --- | --- | --- |
| 工作台 | 全平台统计与管理入口 | 自己负责课程的教学统计 | 进入只读观察工作台 | 本人课程、进度与待办 | 基础工作台 |
| 学校、班级与用户账号 | 管理；创建、导入、编辑、停用、归档、恢复和重置密码 | 只读相关学生 | 只读明确分配学生，主要通过观察端 | 本人资料只读，可修改本人密码 | — |
| 课程列表与详情 | 管理全部课程 | 管理自己创建或受邀授课的课程 | — | 只读有效报名且已发布的课程 | 只读已发布课程列表；当前不能打开课程详情 |
| 课程发布、课时、资源与回放 | 管理 | 管理课程范围内数据 | — | 只读已发布内容并按授权下载或播放 | — |
| 选课导入 | 任意课程 | 自己管理的课程 | — | — | — |
| 报名异常移除 | 管理；仅无作品、评价、反思等业务数据时允许 | — | — | — | — |
| 知识卡片与配套练习编排 | 管理全部课时 | 管理课程范围内课时 | — | — | — |
| 课堂回顾、卡片练习与学习报告 | — | — | 只读观察结果 | 本人学习；每题一次作答 | — |
| 课后任务 | 管理全部任务 | 管理课程范围内任务 | — | 查看并完成本人任务 | — |
| 学生作品 | 查看和评审全部；按保护规则删除或退回 | 查看和评审课程范围内作品；不可物理删除 | 只读明确分配学生的关联作品；观察工作台仅展示已通过成果 | 提交、查看本人作品；仅可删除待评审或被退回的最新版本 | — |
| 学习报告评审 | 查看和评审全部 | 查看和评审课程范围内报告 | 只读明确分配学生的结果 | 查看本人报告状态、评分与评语 | — |
| 教师学情观察 | 可通过观察接口查看 | — | 只读明确分配学生的进度、报告、反思和已通过成果 | — | — |
| 成长档案 | 查看全部；可写成长记录和课程评价 | 查看相关学生且仅含自己课程数据；可写成长记录和课程评价 | 只读明确分配学生的公开结果 | 查看本人档案 | — |
| 独立反思日志 | 不可代写 | 不可代写 | 不可代写 | 本人查看和提交 | — |
| 滑翔机模拟 | 只读全部试飞记录 | 只读自己课程学生的记录 | — | 本人提交试飞并查看结果 | — |
| 灵境小智 | 使用，可读取管理范围内课程上下文 | 使用，仅限负责课程上下文 | — | 使用，仅限有效报名课程上下文 | — |
| 帮助与反馈 | 提交和查看本人反馈；管理全部反馈、内部备注、状态与优先级 | 提交、查看和跟进本人反馈 | 提交、查看和跟进本人反馈 | 提交、查看和跟进本人反馈 | 提交、查看和跟进本人反馈 |
| 通知中心 | 本人通知 | 本人通知 | 本人通知 | 本人通知 | 本人通知 |

### 数据范围与授权原则

| 角色 | 默认数据范围 |
| --- | --- |
| 管理员 | 全平台；个别高风险操作仍受业务保护规则限制，例如有学习记录的账号不可删除、有业务数据的报名不可移除 |
| 执行导师 | 自己创建的课程或被设置为课时授课人的课程，以及历史上参加过这些课程的学生；作品、报告、评价和反思继续限定在课程范围内 |
| 教师 | 仅管理员通过 `users.teacher_id` 明确分配的学生；观察端只读，不能据此获得课程管理、选课、评审或成长记录写入权限 |
| 学生 | 当前账号本人，以及 `enrollments.status='active'` 的已发布课程；被移除报名或课程撤回后不再进入学习视图 |
| 新媒体 | 基础工作台、已发布课程列表、本人反馈和本人通知；当前后端课程详情接口未向该角色开放 |

权限采用“双层校验”：前端 `RoleGuard` 和角色导航负责隐藏、拦截无权限页面，后端 `requireRole`、课程归属、报名关系和对象级策略负责最终授权。不能将“看不到菜单”视为安全边界；直接请求 API 时仍必须通过后端校验。

> 学生选课由**执行导师或管理员**导入，学生端无自助选课；**一经选课不可退课**，误导入仅管理员可经异常修正通道移除。公开注册已关闭，学生账号由管理员导入/创建并在首次登录时强制改密。作品仅由学生本人提交，仅执行导师或管理员可以评审；教师只能查看明确分配学生的结果，不能导入选课、提交、评审或写入成长记录。
> 新课程默认创建为草稿，管理者在课程详情点击「发布课程」后才对学生可见（可随时撤回为草稿）；已发布/已归档课程禁止物理删除，有报名历史（含已移除）的草稿亦不可删除，需留存请归档。
> 学生端一级导航为 7 项（工作台/我的课程/灵境小智/课后任务/我的作品/实验工具/成长档案）；已选课程详情也提供灵境小智入口并自动选中该课程。反思入口在成长档案页、反馈走顶栏按钮、通知走铃铛。移动端侧边栏自动折叠，表格支持横向滚动。
...
- 教师只读查看明确分配学生的成果，其中观察工作台仅展示已通过成果；执行导师按自己创建或授课的课程范围评审，管理员可处理全部作品。

### 课后学习闭环

```text
课堂回顾 → 知识卡片与配套练习 → 学习报告与反思 → 导师评审评分 → 成长档案
```

必修知识卡片完成后才能提交综合任务；全部必需综合任务提交后才能提交学习报告与反思；作品和报告均通过执行导师评审后，课时才记为完成。教师位于这条执行链路之外，只以只读视角观察负责学生的成长结果。

所有已登录角色均可提交反馈、查看自己的反馈、追加说明和确认处理结果，也可以通过顶部铃铛和通知中心接收、筛选及管理站内通知。管理员可查看全部反馈、设置优先级和状态、填写处理结果，并添加仅管理员可见的内部备注。

## 选课与报名规则

- **导入权限**：执行导师（`academic_mentor`）可为自己管理的课程导入全平台学生，管理员可为任意课程导入；教师无选课导入权限，已归档课程不可导入。
...
| `/dashboard/ai` | 灵境小智（基于课程资料的 AI 问答；教师端暂不开放） |
| `/dashboard/ai/settings` | 管理员 AI 配置 |
| `/glider` | 滑翔机模拟实验室（学生试飞；其余角色只读，管理员可查看全部记录） |
| `/courses` | 课程列表 |
| `/courses/create` | 创建课程 |
| `/courses/:id` | 课程详情（课时/课程回放/资源/选课学生；课时含线下场次时间、地点与执行导师；选课学生页签支持导入学生，管理员可移除报名） |
| `/courses/:courseId/ai-knowledge` | 管理员和负责该课程的执行导师管理 AI 知识库 |
| `/courses/:id/edit` | 编辑课程 |
| `/courses/:id/learn` | 课程回顾页（摘要/回放/资料/任务/我的提交） |
| `/courses/:courseId/lessons/:lessonId/learn` | 学生课后学习工作台（课堂回顾、知识卡片与练习、报告与反思、导师评审） |
| `/courses/:courseId/lessons/:lessonId/content` | 执行导师课时学习内容编辑器 |
| `/mentor/content` | 执行导师学习内容编排中心（按课时设置知识卡片与配套习题） |
| `/mentor/reviews` | 执行导师学习报告评审队列 |
| `/mentor/reviews/:reportId` | 学习报告评审详情 |
| `/observer` | 教师只读学情观察工作台 |
| `/observer/students` | 教师负责学生列表 |
| `/observer/students/:studentId` | 教师只读学生学习详情 |
| `/tasks` | 任务总览（学生/执行导师） |
| `/tasks/:id` | 任务详情 |
| `/students` | 学生与用户管理 |
...
| 课程 | `/api/courses` | 课程、课时、任务、资源、回放、选课导入（执行导师/管理员）与管理员移除报名 |
| 课时学习 | `/api/learning` | 学生学习包、卡片练习、报告提交，以及执行导师内容编排 |
| 学习报告评审 | `/api/mentor-reviews` | 执行导师评审队列、详情与通过/退回 |
| 教师观察 | `/api/observer` | 教师负责学生的只读统计、列表与学习详情 |
...
助手默认关闭。管理员配置 DeepSeek API 后才能启用；学生必须选择**有效报名且已发布**的课程，执行导师仅能选择自己负责的课程，教师和新媒体不可使用。课程规定、课时、任务及已发布知识卡片来自数据库；课程上传文件经解析、切分后使用 SQLite FTS5 检索，不依赖额外向量服务。回答可显示来源，补充知识须标明与课程资料的区别。对话仅保留在当前浏览器页面，不在服务器长期保存。

### 以 `deepseek-flash` 为例配置

1. 在 `backend/.env` 设置 `AI_CONFIG_SECRET`。用 `node -e "console.log(require('node:crypto').randomBytes(32).toString('hex'))"` 生成密钥，复制输出的 64 位十六进制值。把它与数据库备份分开安全保存；丢失后已保存的 API Key 无法解密。
2. 重启后端，以管理员身份打开 `/dashboard/ai/settings`。填入 DeepSeek API Key，模型名称填 `deepseek-flash`，API Base URL 填 `https://api.deepseek.com`，按需设置 Prompt、资料检索、来源显示和拓展等级，最后打开“启用 AI 助手”并保存。API Key 加密保存在 SQLite，页面不会回显原值；不要放进前端代码或提交到 Git。
3. 在课程详情打开“管理 AI 知识库”，上传或加入资料，确认状态为“可检索”。学生从已选课程详情进入助手会自动选中该课程。

只有可提取文字的 PDF、DOCX、PPTX、TXT 会进入文件检索。**旧版 DOC/PPT 请先转换为 DOCX/PPTX；扫描版 PDF 请先 OCR 后再上传**。图片、视频等仍可作普通课程资源，但不会进入文字检索。解析失败会显示原因，可重试；可停用或删除文件。已存在的课程资源可从知识库页面加入。单份课程资料上传上限 50 MB。

默认系统 Prompt（管理员可在页面修改）：

> 你是“灵境小智”，本平台的课程学习助手。你帮助学生理解当前选定课程的知识、课时、任务和项目实践，并可解释与课程直接相关的专业知识。使用适合学生学段的清晰中文，先回答问题，再给出可操作的学习建议，不代替学生完成整份作业。
>
...
管理员可配置不同的 HTTPS Base URL，但必须先由服务器管理员将该完整地址加入 `AI_ALLOWED_BASE_URLS`。浏览器始终只请求本项目后端；后端对 DeepSeek 请求设有超时、问题长度和单用户频率限制。请使用 HTTPS 部署站点，避免管理员提交 API Key 时被中途读取。
...
- 灵境小智的文件检索为 SQLite 全文检索，不包含向量检索或扫描文件 OCR；模型回答仍需由学习者核对课程资料。
- 一经选课不可退课：日常无退课入口，误导入仅管理员可异常修正（该生有作品/评价/反思时禁止移除）。
- 成长档案按角色过滤：教师仅见明确分配学生的只读结果；执行导师仅见历史上参加过自己课程（创建或授课）的学生，且作品/评价/反思限定在其课程范围内；**无课程关联的遗留作品/评价/反思（enrollment_id 为空）仅管理员与学生本人可见**。
- 课程评价和成长记录仅执行导师/管理员可写；教师观察端不提供任何业务写操作。
- 作品删除分级保护：学生仅可删除待评审或被退回的最新版本；导师不可物理删除；已通过或有后续版本的作品任何角色不可删。
...
- 课时学习进度由服务端按课堂回顾 25%、知识卡片与配套练习 35%、学习报告与反思 25%、导师评审通过 15% 计算；课堂回顾和每张卡片均需学生明确确认完成。
- 每道卡片练习仅允许学生作答一次；提交后无论正误均显示执行导师预设的答案详解，全部题目完成作答后即可确认完成卡片，答题正确率作为导师评审证据。
- 后端自动化测试覆盖安全隔离、选课闭环、作品权限、档案与评价、学习门控、学习报告版本、导师课程归属、教师观察范围、滑翔机、反馈与通知；前端以 lint、生产构建和角色冒烟为基础自检。
...
- 学生列表、学生详情、档案树、单人档案和按学校/班级批量导出，均限定为执行导师自己创建或受邀授课的课程中有报名记录的学生；搜索或手动填写学生 ID 不能扩大范围。导师也不能为无关学生添加成长记录。
- 保留已移除报名和已归档课程的历史关联；学生停用或归档后，相关导师仍可查看其历史档案。多个课程关联同一学生不会导致列表重复；档案中的课程、作品、评价和反思按导师负责的课程过滤。
- 在自己课程的“导入学生”候选查询中，导师仍可搜索全平台符合条件的学生。没有相关学生时，普通列表和档案树为空，不会退回全平台数据。
- 新增 `studentPolicy.js` 统一导师关系判断；教师仅能只读查看由 `teacher_id` 明确分配的学生，不可写成长记录。本批无数据库结构变更，不需要初始化或重置数据库。

### 执行导师课程管理范围（方案第 8 条）

- 管理员可管理所有课程；执行导师可管理自己创建或受邀授课的课程。课程编辑、发布/撤回、删除、课时和任务创建、资源与回放上传、回放修改/删除、学生导入及候选查询均执行这一规则。导师导入自己负责课程时，仍可在全平台搜索符合条件的候选学生。
- 课程列表、详情页按每门课程的管理权限显示操作入口；无关课程不能打开详情或编辑。资源和视频在接收上传文件前检查权限。
- 本批建立 `backend/policies/coursePolicy.js` 统一管理权限，教师不进入课程执行或选课链路；多人共同负责课程的长期数据模型尚未实现。
- 无数据库结构或启动配置变更，继续使用 README 的启动方式或一键脚本，不需要初始化或重置数据库。
...
- 第 6 条账号写权限及第 7 条删除数据保护已实施；导师学生/档案读取范围与教师明确分配学生的只读范围见权限矩阵，空测试账号身份判定仍待后续整改。
...
“登录账号清单”默认只显示正常账号，可一键切换“已停用 / 已归档 / 全部”，各项显示当前姓名或账号搜索结果中的数量。切换筛选会清空勾选并回到第一页，CSV 导出跟随当前筛选；下方组织结构仍按原方式展示，不受状态筛选影响。
