...
| 学术导师 `academic_mentor` | 课程全流程管理；学生报名；课程资料与回放上传管理；任务总览；作品批改；学生与成长档案管理 |
| 教师 `teacher` | 本校学生管理；任务总览；查看课程回放；查看公开发布作品；本校成长档案 |
| 学生 `student` | 课程浏览与课程回顾（摘要/回放/资料/任务/我的提交）、完成课后任务并提交作品、反思日志、个人成长档案、滑翔机模拟试飞 |
| 新媒体 `media` | 课程浏览（只读）、帮助与反馈、通知中心 |

> 学生选课统一由管理员/学术导师线下报名完成（前端已下线自助选课入口）；**公开注册已关闭**，学生账号由管理员导入/创建（默认密码姓名拼音@123，首登强制改密）。作品上传仅限学生本人（从课后任务入口提交），教师与导师均不参与作品上传；教师仅能查看公开发布（已通过批改）的作品，不参与批改。
> 新课程默认创建为草稿，管理者在课程详情点击「发布课程」后才对学生可见（可随时撤回为草稿）。
> 学生端一级导航为 6 项（工作台/我的课程/课后任务/我的作品/实验工具/成长档案）；反思入口在成长档案页、AI 入口在已选课程详情、反馈走顶栏按钮、通知走铃铛。移动端侧边栏自动折叠，表格支持横向滚动。
...
| `/courses` | 课程列表 |
| `/courses/create` | 创建课程 |
| `/courses/:id` | 课程详情（课时/课程回放/资源/选课学生；课时含线下场次时间、地点与授课教师） |
...
| `/works/:id` | 作品详情 |
| `/archives` | 成长档案 |
| `/archives/reflection` | 反思日志（关联课程与课时） |
| `/feedback` | 我的反馈 |
| `/feedback/new` | 提交反馈 |
...
当前“AI 学习助手”尚未连接大语言模型或外部 AI API。后端根据 PBL、月球、无人机、火星、VR 和反思等关键词返回预设内容，并可附带有限的课程上下文。对外介绍时宜称为“规则式学习助手”。教师端暂不开放（教师不参与课程建设，无课程上下文）。
...
管理员创建或批量导入的学生，默认密码为**姓名拼音 + `@123`**（如 王小明 → `wangxiaoming@123`；姓名不含中文时回退 `pbl123456`），学生首次登录强制修改密码。
...
- 反馈、通知与安全相关模块已有自动化测试；作品等部分业务模块仍缺少集成测试。
- CI 已接入（`.github/workflows/ci.yml`：push/PR 触发后端测试 + 前端 lint/构建）；**main 分支保护需在 GitHub 仓库 Settings → Branches 中开启**（建议勾选 Require pull request + Require status checks 的 CI）。
