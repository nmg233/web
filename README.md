...
第一批对应《账号与权限体系整改方案》第 3 条及第 15 条第 1 项。第二批临时密码整改见下节；学生 CRUD 权限、生命周期及课程/学校 Policy 仍属于后续批次。
...
- 本批对应整改方案第 6 条的账号写权限、第 7 条的删除数据保护，以及相关前端与回归测试。完整停用/归档服务、空测试账号身份判定和导师/教师读取范围仍待后续整改，不代表生命周期整改全部完成。
- 本批无数据库结构变更，沿用上文的本地启动方式和测试账号，无需重置数据库。
